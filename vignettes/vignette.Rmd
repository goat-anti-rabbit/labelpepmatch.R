...
## Table of Contents
  ...
  * [Mass matching peak pairs to a database](#chapter-4) 
    ...
    + [Adding extra identifications to a `pepmatched` object](#4.6)
  ...
  * [Further analysis with Bioconductor limma](#chapter-7)
    ...
    + [Further statistical inference with limma](#7.5)    
...
```{r attach_labelpepmatch,eval=T,echo=T,message=F}
library(labelpepmatch)
```
```{r set_seed,eval=T,echo=F}
set.seed(7)
```
...
Labelpepmatch also has an inbuild summary function that summarizes the data in a somewhat more informative way. This `lpm_summary` function is your best friend throughout the pipeline. It can be applied to any labelpepmatch specific class and will always give you an informative overview of the object. 
...
- <a id="quantdistr"></a>The quantity is an almost symmetric distribution on a log2 scale and there is no clear correllation between mass/charge ratio and quantity. Especially in run B there is a large proportion of features with very low quantity. One can remedy this by only using features that are above a certain quantity threshold. Notice that because of the between-run matched nature of the input data, some features have quantities close to or equal to zero. This can be very helpful in case we want to look for on/off signals, but in many cases, these are just missing features. They can be left out more downstream in the analysis, using one of the filtering parameters.  
...
Next, there is an MA plot where you see that there is a tendency for the light labelled channel to have a higher signal. See [lpm_MAplot](#MAplot) for more information. 
...
```{r pep_massmatch, eval=F,tidy=TRUE,warning=FALSE}
pep.massmatch(input=c(1080.25, 1500.60) ,db=db, ID_thresh=5)
```
```{r inspect_pep_massmatch, eval=T,tidy=TRUE,echo=F,warning=FALSE}
...
#### Adding extra identifications to a `pepmatched` object <a id="4.6"></a>
...
```{r add_extra_identifications,eval=T,fig.width = 12, fig.height = 6}
...
This closer look gives away that we are actually looking at only 3 different peptides, with charges ranging between 2 and 4. Most probabily, since the patterns of expression for the 1644 Da and the 2909 Da peptides align so well, we are looking at a longer isoform or precursor of the Scg-PK-5 peptide (which we already knew from MS/MS, see [add_id](#4.6)). The 790 Da peptide remains unkown, but it may well be a fragment that is also co-encoded in the larger precursor. 
...
<!--note to self: nakijken, volgens mij doet het script het nog niet consistent nu-->
...
```{r normalize_between_arrays,eval=T,tidy=TRUE,fig.width = 9,fig.height=4}
...
#### Further statistical inference with limma<a id="7.5"></a>
Now we know how to create an RG list object and normalize it, there are ample possibilities for linear models on the data. In what follows, we will give some pointers without printing all the output. Here, we use a simple linear model after loess within-array normalization. 
The `lmFit` function offers ample possibilities to for example account for multiple observations of the same peptide in different charge states (ndups, normally used for multiple probes per gene), and different regression methods that can be explored. The `eBayes` function is another very interesting function that should be explored with these kind of data. It was originally designed for inferences on gene expression, where it has proven to be a good ally in the battle against overdispersion.  
